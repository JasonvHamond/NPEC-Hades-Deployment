...
# expected_centers_potato = [(2000, 550, 1)]
# measure_folder(r"F:\Exp35\ROOT2_analysis_V2\final_timeseries", expected_centers_potato)

#folder_dir = r"F:\Exp16\ROOT1_analysis"
#timeline = "53_13_Exp7_Mock_pH7_Col_03_ROOT1_Fish Eye Corrected_A0"
#petri_dish = "53_13_Exp7_Mock_pH7_Col_03_ROOT1_Fish Eye Corrected_A0.png"

# #def plot_dijkstra_exploration(coord_map, explored_nodes, explored_edges, highlight_path=None):
#     """
#     Plot Dijkstra exploration: nodes, edges, and optionally the best path.
#
#     Parameters:
#     - coord_map: dict mapping node IDs to (y, x) coordinates
#     - explored_nodes: set of nodes visited by Dijkstra
#     - explored_edges: set of (u, v) edges explored
#     - highlight_path: list of node IDs forming the selected best path
#     """
#     plt.figure(figsize=(10, 10))
#
#     # All known nodes (gray)
#     for node, (y, x) in coord_map.items():
#         plt.plot(x, y, 'o', color='lightgray', markersize=2)
#
#     # Explored nodes (blue)
...
#
#     # Explored edges (thin blue lines)
...
#
#     # Highlight the best path (red line)
#     if highlight_path:
...
#     plt.axis('equal')
#     plt.title("Dijkstra Explored Nodes and Path")
#     plt.show()

#lengths, paths = nx.single_source_dijkstra(G_biased, start_node, weight='cost')
#explored_nodes = set(paths.keys())
#explored_edges = set()
#for path in paths.values():
#    for u, v in zip(path[:-1], path[1:]):
#        explored_edges.add((u, v))
#best_path = select_best_path(G_biased, start_node, coord_map)
#plot_dijkstra_exploration(coord_map, explored_nodes, explored_edges, highlight_path=best_path)
#
# import matplotlib.pyplot as plt
# import numpy as np
# import cv2
#
# def draw_paths_on_image(image, paths, coord_map, thickness=2):
#     """
#     Draw multiple paths with different colors on an image.
#
#     Parameters:
#         image: np.ndarray (RGB)
#         paths: list of paths (each path is a list of node ids)
#         coord_map: dict {node_id: (y, x)}
#         thickness: line thickness
#     Returns:
#         image with drawn paths
#     """
#     colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255),
#               (255, 255, 0), (255, 0, 255)]  # Red, Green, Blue, Yellow, Magenta
#
#     overlay = image.copy()
#
...
#
# # Get all path infos
# def get_top_n_longest_paths(G, start_node, coord_map, min_dy_ratio=0.8, min_length=2, top_n=5):
#     try:
#         _, paths = nx.single_source_dijkstra(G, start_node, weight='cost')
#     except nx.NetworkXNoPath:
#         return []
...
#         if dy <= 0:
#             continue
#         lateral_deviation = dx / dy
#         total_cost = sum(
#             G.edges[path[i], path[i + 1]].get('cost', 9999.0)
#             for i in range(len(path) - 1)
#         )
#         path_infos.append({
#             'path': path,
...
#     dy_candidates = [p for p in path_infos if p['dy'] >= dy_cutoff]
#     dy_candidates.sort(key=lambda p: p['dy'], reverse=True)
#     return dy_candidates[:top_n]
#
# # Get top 5
# top_paths_info = get_top_n_longest_paths(G_biased, start_node, coord_map, top_n=5)
# top_paths = [p['path'] for p in top_paths_info]
#
# # Draw
# image_with_paths = draw_paths_on_image(image.copy(), top_paths, coord_map, thickness=2)
#
# # Show
# plt.figure(figsize=(10, 10))
# plt.imshow(image_with_paths)
